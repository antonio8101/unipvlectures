 ...
 */
class UniPvEngineeringClassesSpider extends BasicSpider
{
    protected function initialRequests(): array
    {
        $requests = [];

        $urls = Config::get('unipvlecturers.classes_urls') ?? [];
    ...
    public function processItem( ItemInterface $item ): ItemInterface {
        ...
        foreach ($item['internal_nodes_separated'] as $internalNodesSeparated){
            ...
            foreach ($internalNodesSeparated['teachers'] as $t){
                ...
                if (\UnipvLecturers\Models\Teacher::exists($t->name, $t->profile))
                    continue;

                $teacher = new \UnipvLecturers\Models\Teacher();
...
class SaveLessonItemProcessor implements ItemProcessorInterface{
    ...
    public function processItem( ItemInterface $item ): ItemInterface {
        ...
        foreach ( $item['internal_nodes_separated'] as $internalNodesSeparated ) {
            ...
            if ( ! Lesson::exists( $className ) ) {

                $this->createLesson( $className );

            }

            $lesson = Lesson::getOne( $className );

            $teachers = $this->getTeachers( $internalNodesSeparated['teachers'] );

            foreach ($teachers as $t){

                if (TeacherLesson::exists($t->id, $lesson->id))
                    continue;
    ...
    private function getTeachers( array $t ): array {
        ...
        return array_map( fn( $t ) => \UnipvLecturers\Models\Teacher::getOneByNameAndProfile( $t->name, $t->profile ), $t );

    }

    private function createLesson( mixed $className ): void {

        Lesson::create( $className );

    }

    private function createTeacherLesson( ?Lesson $lessonItem, mixed $t ): void {

        $teacherLesson = new TeacherLesson();

        $teacherLesson->lesson_id  = $lessonItem->id;
